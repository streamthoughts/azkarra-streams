...
public class ComponentClassLoader extends URLClassLoader implements ExternalComponentClassLoader {
    ...
    private final URL componentLocation;

    private final ClassLoader systemClassLoader;

    public static ComponentClassLoader newClassLoader(final URL componentLocation,
                                                      final URL[] urls,
                                                      final ClassLoader parent) {
        return AccessController.doPrivileged(
            (PrivilegedAction<ComponentClassLoader>) () -> new ComponentClassLoader(componentLocation, urls, parent)
        );
    }

    /**
     * Creates a new {@link ComponentClassLoader} instance.
     *
     * @param componentLocation the top-level component location.
     * @param urls              the URLs from which to load classes and resources.
     * @param parent            the parent {@link ClassLoader}.
     */
    private ComponentClassLoader(final URL componentLocation,
        ...
        synchronized (getClassLoadingLock(name)) {
            ...
            if (loadedClass == null && shouldLoadFromUrls(name)) {
                try {
                    ...
                    LOG.trace(
                        "Class '{}' not found in componentLocation {}. Delegating to parent",
                        name,
                        componentLocation
                    );
                }
