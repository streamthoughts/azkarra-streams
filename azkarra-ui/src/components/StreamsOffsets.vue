...
*/
<template>
  <div id="component-streams-offsets">
    <div class="row justify-content-between align-items-center">
      <div class="col">
        <p class="mb-1">This only displays consumer-lags for the topics and partitions currently assigned to this Kafka
          Streams instance.</p>
      </div>
      <div class="col-3">
        <div class="fl-right">
          ...
          <table class="table">
            ...
            <tr class="d-flex">
              ...
              <th class="col-md-2">
                <a data-toggle="tooltip" data-placement="top"
                   title="The offset of the last message consumed by the consumer for a specific partition.">
                  Current Offset <i class="fas fa-info-circle"></i>
                </a>
              </th>
              <th class="col-md-2">
                <a data-toggle="tooltip" data-placement="top"
                   title="The last offset committed by the consumer for a specific partition.">
              ...
              <th class="col-md-3">
                <a data-toggle="tooltip" data-placement="top"
                   title="The number of messages the consumer lags behind the producer by for a specific partition (i.e : EndOffset - CurrentOffset - 1)  ">
            ...
            <template v-for="offset in topic.offsets" :key="offset.id">
              <tr class="d-flex">
                ...
                <td class="col-md-3">
                  <div style="text-align:center">{{ offset.lag }} <span
                      v-if="offset.lag > 0">(Estimated Lag Time : {{ offset.lag_time }})</span></div>
...
export default {
  ...
  methods: {
    load() {
      var that = this;
      streamApiV1.getInstanceOffsetsById(this.id).then(response => {
        let mapOffsetGroupedByTopic = response.data.consumers.flatMap(consumer => {
        ...
        that.topics = _.orderBy(Array.from(mapOffsetGroupedByTopic).map(([key, value]) => ({
          name: key,
          offsets: _.orderBy(value, 'id')
        }), 'name'));
      });
    }
  }
