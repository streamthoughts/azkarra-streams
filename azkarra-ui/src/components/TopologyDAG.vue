        ...
        <div id="streams-topology-generals">
           <ul class="list-inline left-border">
              <li class="list-inline-item">
                <strong>sub-topologies : {{subTopologyCount}}
              </li>
              <li class="list-inline-item">
                <strong>topic sources :</strong> {{sourceCount}}
              </li>
              <li class="list-inline-item">
                <strong>topic sinks :</strong> {{sinkCount}}
              </li>
              <li class="list-inline-item">
                <strong>stores :</strong> {{storeCount}}
              </li>
           </ul>
        </div>
    ...
    methods: {
        ...
        buildTopologyGraph(json) {
            ...
            json.subTopologies.forEach(sub => {
                sub.nodes.forEach( node => {
                    if (node.type == "SOURCE") { node.topics.forEach( topic => sourceTopics.push(topic)); }
                });
            });
                ...
                sub.nodes.forEach( (node, i) => {
                    ...
                    if (node.type == "SOURCE") {
                        node.topics.forEach( topic => {
                            let objectTopicNode = topicNodes.get(topic) || new TopicNode({ name : topic, subTopology : sub.id });
                    ...
                    if (node.type == "SINK") {
                        ...
                        let objectTopicNode = new TopicNode({ name : node.topic, isIntermediate : isIntermediate, subTopology : sub.id});
                        nodesKeyedByName.set(objectTopicNode.name, objectTopicNode);
                        node.successors.push(node.topic);
                    }
                });

                let layouts = this.buildTopologyNodes(nodesKeyedByName, Array.from(topicNodes.values()));

                let levels = layouts.map( (nodes, i) => new SubTopologyLevel({ id : i, subTopology : sub.id, nodes : nodes}));
                topologiesLayouts.push(
                    new SubTopology({id:sub.id, levels : levels, stores: Array.from(stores.values())})
                );
            });
        ...
        createSVG(objectTopology) {
            ...
            defs.selectAll('marker')
                .data([{ id: 2, name: 'arrow', path: 'M 0,0 m -5,-5 L 5,0 L -5,5 Z', viewbox: '-5 -5 10 10' }])
                .enter()
                .append('svg:marker')
                .attr('id', 'arrow')
                .attr('markerHeight', 5)
            ...
            let icon = defs.append("g")
                .attr("id", "store-icon");
            icon.append("path")
                    .attr("d", "M 1 17 C 1 -4.33 96 -4.33 96 17 L 96 65 C 96 86.33 1 86.33 1 65 Z")
                    .attr("fill", "#ffffff")
                    .attr("stroke", "#000000")
                    .attr("stroke-width", 2);
            icon.append("path")
            ...
            objectTopology.subTopologies.forEach( d => {
                d.coordinates = {
                    x : d.id * (config.topologyWidth + config.topologyMarginRight),
            ...
            var gSubTopologies = g
                ...
                .enter()
                .append("g")
                .attr("id", function(d) { return "sub-topology-" + d.id })
            ...
            gSubTopologies.append("rect")
                ...
                .attr("height", function(d, i) { return d.levels.length * 120 })
                .attr("rx", 10).attr("ry", 10)
                .classed("sub-topology", true);

            // add sub-topology name
            gSubTopologies.append("text")
                .attr("x", config.topologyWidth - 10)
                .attr("y", 20)
                .attr("dy", ".30em")
                .text(function (d) { return "sub-topology-" + d.id})
                .attr("text-anchor", "end")
                .classed("text-sub-topology-id", true);

            // add groups for containing all sub-topology's nodes
            var gSubTopology = gSubTopologies.append("g")
                .attr("id", function(d) { return "g-sub-topology-" + d.id + "-nodes"})
            ...
            gSubTopology.each(function(subTopology){
                ...
                let gStore = d3.select(this).selectAll(".g-sub-topology-store")
                    .data(function(d) { return d.stores })
                    .enter()
                    .append("g")
                    .attr("transform", function(d, i) {
                        return "translate(" + storeTranslateX + ", " + (storeTranslateY + i *  100) + ")"
                    })
                    .each(function (d, i) {
                        d.coordinates = {
                        x : subTopology.coordinates.x + storeTranslateX + 50,
                        y : subTopology.coordinates.y + 20 + (storeTranslateY + i *  100) }
                    })
                    .classed("g-sub-topology-store", true)
                gStore.append("use").attr("xlink:href","#store-icon");

                gStore.append("circle")
                        .attr("cx", 50)
                        .attr("cy", 20)
                        .attr("r", 7)
                        .style("fill", "black")
                        .attr("id", function (d) { return "anchor-" + d.name.toLowerCase()})
                ...
                var gSubTopologyLevels = d3.select(this).selectAll(".g-sub-topology-level")
                    .data(function (d) { return d.levels })
                    .enter()
                    .append("g")
                    .classed("g-sub-topology-level", true)
                    .attr("id", function (d, i) { return "g-sub-topology-" + subTopology.id + "-level-" + i })
                    .each( function(d, i) {
                        let translateY = 50 + (i * 100 );
                        d.coordinates = { x : subTopology.coordinates.x, y : subTopology.coordinates.y + translateY }
                    })
                    .attr("transform", function(d, i) {
                        return "translate(0, " + (50 + (i * 100 )) + ")";
                    });

                gSubTopologyLevels.selectAll(".g-sub-topology-nodes")
                    .data(function(d){ return d.nodes.filter(node => node.isDrawable()) })
                    .enter()
                    .append("g")
                    .attr("class", function (d) { return "g-sub-topology-node g-sub-topology-node-" + d.type.toLowerCase()});

                gSubTopologyLevels.each(function(level, i){
                    d3.select(this).selectAll(".g-sub-topology-node").each(function(d, i){
                        let x = 20 + (config.topologyNodeWith / level.nodes.length) * i;
                        let width = (config.topologyNodeWith / level.nodes.length) - 5;
                        ...
                        d3.select(this)
                            ...
                            .text(function(d) { return d.normalizedName() })
                            .classed("topology-node", true);
                        ...
                        d3.select(this)
                            .datum(function(d) {
                                d.coordinates = {x : level.coordinates.x + cx, y : level.coordinates.y + cy};
                                return d;
                            })
                            .append("circle")
                            .attr("cx", cx)
            ...
            g.selectAll(".g-sub-topology-node").each(function (source) {
                source.successors.forEach( s => {
                    ...
                    let x2 = (target.coordinates.y > source.coordinates.y) ? target.coordinates.x : target.coordinates.x - 10;
                    let y2 = (target.coordinates.y > source.coordinates.y) ? target.coordinates.y -10 : target.coordinates.y;
                    links.push({source: {x : x1 , y : y1}, target: {x : x2, y : y2} });
                });
            });
...
class TopicNode extends TopologyNode {
    constructor(data) {
        super({ name : data.name, type : "TOPIC", successors : data.successors, predecessors : data.predecessors} );
