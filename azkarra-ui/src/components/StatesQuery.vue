              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <input v-model:value="query.options.retry_backoff_ms"
                         type="text"
                         class="form-control"
                         id="query-options-retry-backoff"
                         placeholder="in milliseconds">
                </div>
              </div>
              <div class="row mb-3">
              ...
              <div class="row mb-3 justify-content-md-center">
                <div class="col-md-auto">
                  <button v-on:click.prevent="execute()"
                          v-bind:disabled="isActionBtnDisabled()" class="btn btn-primary">Execute
                  </button>
                </div>
                <div class="col-md-auto">
                  <button v-on:click.prevent="copyAsCurl()"
                          v-bind:disabled="isActionBtnDisabled()" class="btn btn-secondary">CopyAsCurl
                  </button>
                </div>
              </div>
            </form>
...
export default {
  ...
  data: function () {
    return {
      ...
      stores: [
        ...
        {
          typeLabel: 'TimestampedKeyValue',
          typeValue: 'timestamped_key_value',
          operations: keyValueOperations,
          isTimestamped: true
        },
        {typeLabel: 'Window', typeValue: 'window', operations: windowOperations},
        {
          typeLabel: 'TimestampedWindow',
          typeValue: 'timestamped_window',
  ...
  created() {
    this.loadApplicationsIds()
  },
  watch: {
    '$route': 'loadApplicationsIds'
  },
  updated() {
  ...
  methods: {
    loadApplicationsIds() {
      let that = this;
      applicationApiV1.getAllApplications().then(response => that.applications = response.data);
    },

    fetchApplicationMetadata() {
      let that = this;
      applicationApiV1.getApplicationById(this.query.application).then(response => {
        let stores = new Set();
        response.data.instances.forEach(instance => {
          instance.metadata.stores.forEach(store => stores.add(store));
          that.applicationServers.push(instance.endpoint.address + ":" + instance.endpoint.port);
        });
        that.stateStoreNames = stores;
      });
    },
    ...
    copyAsCurl() {
      ...
      input.setAttribute('value', applicationApiV1.getQueryStateStoreAsCurl(this.buildQuery()));
    ...
    execute() {
      ...
      applicationApiV1.queryApplicationStateStore(that.buildQuery()).then(response => that.response = response.data);
    },
