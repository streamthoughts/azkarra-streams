...
public class WindowGetAllQuery<K, V> extends DecorateQuery<Query> implements LocalExecutableQuery<Windowed<K>, V> {
     ...
     */
    WindowGetAllQuery(final String store) {
    ...
    public Try<List<KV<Windowed<K>, V>>> execute(final LocalStoreAccessProvider provider, final long limit) {
        ...
        final Reader<ReadOnlyWindowStore<K, V>, List<KV<Windowed<K>, V>>> reader =
            reader().map(iterator -> LocalExecutableQuery.toKeyValueListAndClose(iterator, limit));
